...
def get_override_total(groups, assessments, student_id):
    ...
    for assessment in assessments:
        flex = assessment.flexassessment_set.filter(user_id=student_id).first().flex
...
def get_difference(groups, assessments, student_id):
    diff = get_override_total(groups, assessments, student_id) - get_default_total(groups, student_id)
    return round(diff, 2)