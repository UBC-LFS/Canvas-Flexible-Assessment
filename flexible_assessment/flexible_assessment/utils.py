...
def update_students(request, course):
    ...
    students_canvas = (
        FlexCanvas(request).get_course(course.id).get_users(enrollment_type="student")
    )
    students_canvas = list(students_canvas)
    students_db = models.UserProfile.objects.filter(
        usercourse__role=models.Roles.STUDENT, usercourse__course=course
    )
...
def set_course(course_id, course_name):
    ...
    if not course_set.exists():
        logger.info(
            "New Course Created:", extra={"course": course_name, "user": course_id}
        )
