...
class TestUtils(TestCase):
    ...
    def test_add_new_students(self, mocked_flex_canvas_instance):
        ...
        student_profiles = models.UserProfile.objects.filter(
            display_name__contains="student"
        )

        def mock_get_users(self, **kwargs):
            return [
                mock_classes.MockUser(student.display_name, student.pk)
                for student in list(student_profiles)
            ]

        mock_course = mocked_flex_canvas_instance.get_course(1)
        mock_course.get_users = types.MethodType(
            mock_get_users, mock_course
        )  # Add the get_users method to this instance of MockCanvasCourse

        utils.update_students("request", course)

        users_after = models.UserCourse.objects.filter(course=course_id)
        self.assertEqual(
            users_after.count(), student_profiles.count() + 1
        )  # all students + 1 instructor are now in course
    ...
    def test_remove_students(self, mocked_flex_canvas_instance):
        ...
        non_students = models.UserCourse.objects.filter(course=course_id).exclude(
            role=models.Roles.STUDENT
        )
        users_after = models.UserCourse.objects.filter(course=course_id)
        self.assertEqual(
            users_after.count(), non_students.count()
        )  # There are no more students in the course
    ...
    def test_set_user_profile(self):
        ...
        user = utils.set_user_profile(
            non_existant_key, "New Login Id", "New Display Name"
        )
        ...
        self.assertEqual(user.login_id, "Some ID")
        self.assertEqual(user.display_name, "Some Name")
    ...
    def test_set_user_course_enrollment(self):
        ...
        user_course_set = models.UserCourse.objects.filter(
            user_id=existing_student.user_id, course_id=course_id
        )
        self.assertFalse(user_course_set.exists())

        utils.set_user_course_enrollment(
            existing_student, existing_course, models.Roles.STUDENT
        )
