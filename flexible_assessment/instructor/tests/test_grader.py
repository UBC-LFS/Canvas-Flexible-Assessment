...
class TestGrader(TestCase):
    ...
    def bulk_create_flexes_for_student(self, student, assessments, flexes):
        ...
        """
        flex_assessments = [
            FlexAssessment(user=student, flex=flexes[index], assessment=assessment)
    ...
    def test_Grader_determines_if_flex_is_valid_correctly(self):
        ...
        self.bulk_create_flexes_for_student(
            students[1], assessments, [10, 20, 70, None]
        )
    ...
    def test_Grader_calculates_override_total_correctly(self):
        ...
        self.bulk_create_flexes_for_student(
            students[0], assessments, [50, 0, 10, 40]
        )  # This matches the default weights
        self.bulk_create_flexes_for_student(
            students[1], assessments, [100, None, None, None]
        )
        self.bulk_create_flexes_for_student(
            students[2], assessments, [60, 20, 10, 10]
        )  # Different from default weights
        self.bulk_create_flexes_for_student(
            students[3], assessments, [50, 10, 20, 1]
        )  # This doesn't sum to 100

        self.assertEqual(
            grader.get_override_total(groups_dict, students[0], course), 84.00
        )
        self.assertEqual(
            grader.get_override_total(groups_dict, students[1], course), None
        )
        self.assertEqual(
            grader.get_override_total(groups_dict, students[2], course), 40.20
        )
        self.assertEqual(
            grader.get_override_total(groups_dict, students[3], course), None
        )
