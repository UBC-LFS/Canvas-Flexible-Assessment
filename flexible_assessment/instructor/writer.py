...
def grades_csv(course, students, groups):
    """Creates csv response for final grade list, using bulk-loaded data."""
    ...
    # --- Preload all flexes for these students & this course in one query ---
    ...
    def compute_override_total_and_flag(student_id: int):
        """
        Override total using flex weights, if the student has a valid flex:
          - all flexes non-null
          - sum of flexes == 100
        Returns (override_total or None, chose_flag).
          - chose_flag == True means valid flex (like old valid_flex).
          - If invalid, override_total is None.
        """
        flex_values = []
        flex_sum = Decimal("0")
        has_null = False
        ...
        for assessment in assessments:
            ...
            if fa_flex is None:
                ...
                flex_val = Decimal(str(fa_flex))
                flex_values.append(flex_val)
                flex_sum += flex_val
