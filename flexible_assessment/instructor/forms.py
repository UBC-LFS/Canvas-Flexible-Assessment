...
class CourseSettingsForm(ModelForm):
    ...
    def clean(self):
        ...
        if open_datetime > close_datetime:
            self.add_error(
                None, ValidationError("Close date should be after open date")
            )
...
class AssessmentBaseFormSet(BaseModelFormSet):
    ...
    def clean(self):
        ...
        default_sum = sum([form.cleaned_data.get("default", 0) for form in self.forms])
        if default_sum != 100:
        ...
        all_assessments = (
            []
        )  # Example: {'title': 'assignment1', 'default': Decimal('20.00'), 'min': Decimal('10.00'), 'max': Decimal('30.00'), 'id': <Assessment: assignment1, test_course1>}
        total_min = 0
        total_max = 0
        ...
        for form in self.forms:
            ...
            allocations = [("default", default), ("max", max_value), ("min", min_value)]
            ...
            for field, allocation in allocations:
                if allocation and (allocation > 100.0 or allocation < 0.0):
                    form.add_error(
                        field,
                        ValidationError(
                            "{} must be within 0.0 and 100.0".format(labels[field])
                        ),
                    )
            if "<" in title or ">" in title:
                form.add_error(
                    "title", ValidationError("Invalid special character in title")
                )
...
def get_assessment_formset(extra=0):
    AssessmentFormSet = modelformset_factory(
        ...
        extra=extra,
        widgets={
            "title": forms.TextInput(attrs={"size": 15}),
